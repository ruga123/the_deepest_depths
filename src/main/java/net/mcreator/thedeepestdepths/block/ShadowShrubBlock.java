...
public class ShadowShrubBlock extends TheDeepestDepthsModElements.ModElement {
	@ObjectHolder("the_deepest_depths:shadow_shrub")
	public static final Block block = null;
	public ShadowShrubBlock(TheDeepestDepthsModElements instance) {
		super(instance, 417);
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
			feature = new DefaultFlowersFeature(BlockClusterFeatureConfig.field_236587_a_) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, BlockClusterFeatureConfig config) {
					...
					if (dimensionType == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("the_deepest_depths:shadow_dimension")))
			...
			event.getRegistry().register(feature.setRegistryName("shadow_shrub"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("the_deepest_depths:shadow_shrub"), configuredFeature);
		}
	}
	@SubscribeEvent
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		boolean biomeCriteria = false;
		if (new ResourceLocation("the_deepest_depths:shadow_desert").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
	...
	public static class BlockCustomFlower extends FlowerBlock {
		...
		public boolean isValidGround(BlockState state, IBlockReader worldIn, BlockPos pos) {
			...
			return (block == ShadowSandBlock.block.getDefaultState().getBlock());
		}
