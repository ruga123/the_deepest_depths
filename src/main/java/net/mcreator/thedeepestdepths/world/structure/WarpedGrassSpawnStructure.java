...
public class WarpedGrassSpawnStructure extends TheDeepestDepthsModElements.ModElement {
	private static Feature<NoFeatureConfig> feature = null;
	private static ConfiguredFeature<?, ?> configuredFeature = null;
	public WarpedGrassSpawnStructure(TheDeepestDepthsModElements instance) {
		super(instance, 410);
	...
	private static class FeatureRegisterHandler {
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
					...
					if ((random.nextInt(1000000) + 1) <= 1000000) {
						...
						for (int a = 0; a < count; a++) {
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("the_deepest_depths", "warpedgrass"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("warped_grass_spawn"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("the_deepest_depths:warped_grass_spawn"),
					configuredFeature);
		}
	}
	@SubscribeEvent
